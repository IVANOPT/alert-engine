...
/**
  * Created by Ivan on 06/28/2018.
  */
class BaselineForStoreFunction[T <: AlertPublishService[DStream[JSONObject], DStream[JSONObject]]](alertService: T)
  extends RealTimeDataCalculator {

  private final val log: org.slf4j.Logger = org.slf4j.LoggerFactory.getLogger(this.getClass)

  alertService.register(BASELINE_STORE, this)
  ...
  override def handle(record: JSONObject, field: String): (String, Double) = {
    if (null == record.get(field)) {
      log.error(s"BaselineForStoreFunction.handle() | record:$record | Field:$field is not exists")
      return null
    }
