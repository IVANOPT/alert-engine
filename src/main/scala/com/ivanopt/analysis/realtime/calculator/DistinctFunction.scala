...
/**
  ...
  */
class DistinctFunction[T <: AlertPublishService[DStream[JSONObject], DStream[JSONObject]]](alertService: T)
  ...
  alertService.register(DISTINCT, this)
  ...
  override def calculate(rule: JSONObject, ds: DStream[JSONObject]): DStream[JSONObject] = {
    ...
    val distinctCountDS = distinctFieldDS.count()
      .map(line => (field, line))
    val resultDS = distinctCountDS.map(pair => assemblyValue(rule, pair))
      resultDS
  }

  override def handle(record: JSONObject, field: String): (String, Double) = {
    if (null == record.get(field)) {
      log.error(s"DistinctFunction.handle() | record:$record | Field:$field is not exists")
      return null
    }
  ...
  override def assemblyValue(rule: JSONObject, value: Any): JSONObject = {
    ...
    ruleParser.put("finalValue", value.asInstanceOf[(String, Long)]._2)
