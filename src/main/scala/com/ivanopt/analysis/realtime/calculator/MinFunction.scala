...
class MinFunction[T <: AlertPublishService[DStream[JSONObject], DStream[JSONObject]]](alertService: T)
  ...
  override def handle(record: JSONObject, field: String): (String, Double) = {
    if (null == record.get(field)) {
      log.error(s"MinFunction.handle() | record:$record | Field:$field is not exists")
      return null
    }
